...
import useGetUserStat from "../../Hooks/useGetUserStat";
    ...
    return (
        <div className="w-full h-[100%] text-center">
            <div className="flex flex-col justify-center items-center gap-3 p-5">
                <img className="h-[100px] w-[100px] rounded-full border-2 border-green-300" src={userStats?.user?.photo} alt="" />
                <h1 className="font-semibold">{userStats?.user?.name}</h1>
                <h1 className="font-semibold">{userStats?.user?.email}</h1>
                <button className="btn-secondary rounded-md py-2 px-3 font-semibold">Update Profile</button>
